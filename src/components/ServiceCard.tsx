...
export function ServiceCard({
  ...
  return (
    ...
    >
      <span
        className="mb-4 inline-flex h-12 w-12 items-center justify-center rounded-full bg-ltm-navy text-white"
        aria-hidden="true"
      >
        <Icon className="h-6 w-6" />
      </span>
      <h3 className="text-lg font-semibold text-ltm-navy">{title}</h3>
      <p className="mt-2 flex-1 text-sm leading-relaxed text-ltm-slate">
        {description}
      </p>
      <span className="mt-4 inline-flex items-center gap-1 text-sm font-medium text-ltm-navy group-hover:underline">
        Learn more
        <ArrowRight
